  ...
  try {
    ...
    if (typeof amount !== "number" || amount <= 0 || !type) {
      res.status(400).json({
        status: "fail",
        message:
          "Số tiền phải là số lớn hơn 0 và cần chọn loại giao dịch (expense/income)",
      });
      return;
    }
    ...
    res
      .status(500)
      .json({ status: "error", message: "Lỗi hệ thống", error: error.message });
  }
};
...
export const editTransaction = async (
  ...
  try {
    ...
    if (!id || !mongoose.Types.ObjectId.isValid(id)) {
      res
        .status(400)
        .json({ status: "fail", message: "ID giao dịch không hợp lệ" });
      return;
    }
    ...
    const updateTran = await transaction.findOneAndUpdate(
    ...
    if (!updateTran) {
      res.status(404).json({
        status: "fail",
        message: "Giao dịch không tồn tại hoặc bạn không có quyền chỉnh sửa",
      });
      return;
    }
    res
      .status(200)
      .json({ status: "success", message: "Cập nhật giao dịch thành công" });
  } catch (error: any) {
...
export const deleteTransaction = async (
  ...
  try {
    ...
    const deleteTran = await transaction.findOneAndDelete({
      _id: id,
      user_id: userId,
    });
    if (!deleteTran) {
    ...
    res
      .status(200)
      .json({ status: "success", message: "Xóa giao dịch thành công" });
  } catch (error: any) {
  ...
  try {
    ...
    if (date) {
      ...
      if (isNaN(queryDate.getTime())) {
        res
          .status(400)
          .json({ status: "fail", message: "Định dạng ngày không hợp lệ" });
        return;
      }
    ...
    const groupedMap = transactions.reduce(
      (acc: Record<string, GroupedDayTransactions>, item: any) => {
...
export const getCategoryPieChart = async (
  req: AuthenticatedRequest,
  res: Response,
) => {
  try {
...
export const getMonthlySpendingTrend = async (
  ...
  try {
    const userId = req.user?.userId;
    const userObjectId = new mongoose.Types.ObjectId(userId);
    const monthsLimit = parseInt(req.query.months as string) || 5;
    const now = new Date();
    const startDate = new Date(
      now.getFullYear(),
      now.getMonth() - monthsLimit + 1,
      1,
      0,
      0,
      0,
    ...
    res.status(200).json({
      status: "success",
      message: "Lấy dữ liệu chi tiêu hàng tháng thành công",
      data: {
        trendData,
      },
    });
  } catch (error: any) {
    console.error("Lỗi getMonthlySpendingTrend:", error);
    res.status(500).json({
      status: "error",
      message: "Lỗi hệ thống",
      error: error.message,
